...
def calculate_f1_score(ground_truth, predictions):
    ...
    ground_truth = [str(item) if item is not None else '' for item in ground_truth]
    predictions = [str(item) if item is not None else '' for item in predictions]
...
def pipeline_tuning(lr, hiddenLayers, batchSize, maxEpochs):
    ...
    nerTagger = NerDLApproach()\
                  .setInputCols(["sentence", "stem", "embeddings"])\
                  .setLabelColumn("label")\
